...
export function CombatantFigure({ id, label, side, animation = 'idle' }: { id: string; label: string; side: 'player' | 'enemy'; animation?: CombatAnimation }) { const shape = SHAPES[id] ?? SHAPES.ashling; return <figure className={`combatant-figure figure-${side} action-${animation}`} aria-label={label}><svg viewBox="0 0 160 180" role="img"><title>{label}</title><ellipse className="figure-shadow" cx="80" cy="157" rx="58" ry="10" /><g className="figure-art"><path className="figure-body" d={shape.body} /><path className="figure-accent" d={shape.accent} /><path className="figure-mark" d={shape.mark} /></g></svg></figure>; }
