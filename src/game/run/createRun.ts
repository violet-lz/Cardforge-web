...
export function createRun(seed: number, now = Date.now(), character: CharacterDefinition = CHARACTERS.wanderer, ascensionLevel = 1, starterCardUpgradeIndexes: readonly number[] = []): GameState {
  const normalizedSeed = seed >>> 0; const difficulty = clampDifficulty(ascensionLevel); const upgraded = new Set(starterCardUpgradeIndexes); const deck = character.startingDeck.map((id, index) => ({ ...createCardInstance(id, `${character.id}-start-${index}`), upgraded: upgraded.has(index), upgradeLevel: upgraded.has(index) ? 2 : 0 }));
