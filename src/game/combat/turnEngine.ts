...
export function beginPlayerTurn(state: CombatState, rng: SeededRng, drawCount = 5): CombatState { if (state.phase !== 'player-turn' && state.phase !== 'enemy-turn') throw new Error('Cannot begin a turn after combat ends'); const prepared = { ...state, player: syncLegacyStatusFields({ ...state.player, block: 0, statuses: expireStatuses(state.player.statuses, 'turn-start') }), enemies: state.enemies.map((enemy) => syncLegacyStatusFields({ ...enemy, statuses: expireStatuses(enemy.statuses, 'turn-start') })), energy: refreshEnergy(state.energy) }; const result = drawCards(prepared.deck, drawCount, rng); return { ...prepared, turn: prepared.turn + 1, phase: 'player-turn', deck: result.deck, lastDrawnCardUids: result.drawn.map((card) => card.uid), log: [...prepared.log, `Turn ${prepared.turn + 1} start`, `Drew ${result.drawn.length} card(s)`] }; }
