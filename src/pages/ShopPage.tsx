...
function itemInfo(item: ShopItem, t: (key: string, values?: Record<string, string | number>) => string) {
 if (item.kind === 'card') { const card = resolveCard(item.id); return { name: card?.name ?? item.id, kind: t('shop.card'), detail: card ? `${cardPresentation(card).type} · ${cardPresentation(card).rarity} · ${t('shop.cost', { cost: card.cost })}` : t('shop.unknown'), description: card?.description }; }
 if (item.kind === 'potion') { const potion = resolvePotion(item.id); return { name: potion?.name ?? item.id, kind: t('shop.potion'), detail: t('shop.inCombat'), description: potion?.description }; }
 ...
 const relic = resolveRelic(item.id); return { name: relic?.name ?? item.id, kind: t('shop.relic'), detail: t('shop.relicDetail'), description: relic?.description };
}
