...
function itemInfo(item: ShopItem, t: (key: string, values?: Record<string, string | number>) => string, cl: ReturnType<typeof import('../app/useContentLocale').useContentLocale>, locale: Locale) {
 if (item.kind === 'card') { const card = resolveCard(item.id); return { name: card ? cl.cardName(card) : item.id, kind: t('shop.card'), detail: card ? `${cardPresentation(card, locale).type} · ${cardPresentation(card, locale).rarity} · ${t('shop.cost', { cost: card.cost })}` : t('shop.unknown'), description: card ? cl.cardDesc(card) : undefined }; }
