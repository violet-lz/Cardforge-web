...
import { localizedBiome, localizedLocation } from '../app/contentLocale';
...
export function RunPage({ state, onBack, onEnterNode }: RunPageProps) {
 ...
 const map = state.map; const [previewId, setPreviewId] = useState<string | null>(null); const character = CHARACTERS[state.player?.characterId ?? 'wanderer']; const previewNode = map?.nodes.find((node) => node.id === previewId); const currentNode = map?.nodes.find((node) => node.id === map.currentNodeId); const activeNode = previewNode ?? currentNode; const biome = biomeById(activeNode?.regionId ?? map?.biomeId); const biomeName = (locale === 'en' ? localizedBiome(biome.id, locale).name : '') || biome.name; const biomeTagline = (locale === 'en' ? localizedBiome(biome.id, locale).tagline : '') || biome.tagline; const regionChain = [...new Set(map?.nodes.slice().sort((a, b) => a.row - b.row).map((node) => node.regionId ?? map.biomeId) ?? [])].map((id) => biomeById(id)); const depth = Math.max(1, ...(map?.nodes.map((node) => node.row + 1) ?? [1])); const canvasHeight = mapCanvasHeight(depth, compactMap, narrowMap); const mapStyle = { '--map-depth': String(depth) } as CSSProperties; const visibleEdges = map ? uniqueEdges(map.edges) : [];
 return <main className={`run-shell ${biome.accentClass}`}><header className="run-header"><div><p className="eyebrow">{t('run.act', { act: state.run?.currentAct ?? 1 })} · {biomeName} · {t('run.difficulty', { level: state.run?.ascensionLevel ?? 1 })} · SEED {state.seed}</p><h1>{t('run.title')}</h1><p className="subtitle">{biomeTagline}</p></div><button className="text-button" onClick={onBack}>{t('backHome')}</button></header><nav ref={routeStripRef} className={`region-route ${routeDragging ? 'dragging' : ''}`} aria-label={t('run.routeLabel')} onPointerDown={onRoutePointerDown} onPointerMove={onRoutePointerMove} onPointerUp={endRoutePointer} onPointerCancel={endRoutePointer}>{regionChain.map((region, index) => { const rName = (locale === 'en' ? localizedBiome(region.id, locale).name : '') || region.name; return <span key={region.id} className={`${region.accentClass} ${region.id === biome.id ? 'current' : ''}`}><b>{index + 1}</b>{rName}</span>; })}</nav><section className="region-vista" aria-label={`${biomeName}`}><RegionScene regionId={activeNode?.regionId ?? map?.biomeId} /><div className="region-vista-caption"><strong>{biomeName}</strong><span>{biomeTagline}</span></div></section><section className="run-grid"><article className="panel map-panel"><div className="section-heading"><div><p className="eyebrow">CINDERLAND SURVEY</p><h2>{t('run.mapTitle')}</h2><p className="map-helper">{t('run.mapHelper')}</p></div><span className="route-count">{map?.visitedNodeIds.length ?? 0}/{map?.nodes.length ?? 0}</span></div>
  <div className="map-canvas topographic-map" style={mapStyle}><CartographicTerrain /><svg className="map-links" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">{map && visibleEdges.map((edge) => { const from = map.nodes.find((candidate) => candidate.id === edge.from); const to = map.nodes.find((candidate) => candidate.id === edge.to); if (!from || !to) return null; const active = map.visitedNodeIds.includes(from.id) && map.availableNodeIds.includes(to.id); const traveled = map.visitedNodeIds.includes(from.id) && map.visitedNodeIds.includes(to.id); const future = !active && !traveled; const classes = `${active ? 'active' : ''} ${traveled ? 'traveled' : ''} ${future ? 'future' : ''}`; const path = routePath(from, to, canvasHeight, compactMap, narrowMap); return <g key={`${edge.from}-${edge.to}`} data-route-from={edge.from} data-route-to={edge.to} className={classes}><path className="map-link" d={path} /></g>; })}</svg><div className="map-nodes-free">{map?.nodes.map((node) => { const available = map.availableNodeIds.includes(node.id); const visited = map.visitedNodeIds.includes(node.id); const current = map.currentNodeId === node.id; const presentation = nodePresentation(node, available, visited, current, locale); const position = point(node); const locName = localizedLocation(node.regionId ?? map.biomeId, node.locationName, locale); return <button key={node.id} data-node-id={node.id} data-connections={node.connections.join(' ')} data-map-row={node.row} data-map-column={node.column} className={`node-button node-${node.type} landmark-${node.locationKind} node-status-${presentation.state} ${biomeById(node.regionId ?? map.biomeId).accentClass}`} style={{ left: `${position.x}%`, top: `${position.y}%` }} aria-label={`${presentation.aria}，${locName}`} disabled={presentation.state !== 'available'} onFocus={() => setPreviewId(node.id)} onMouseEnter={() => setPreviewId(node.id)} onClick={() => onEnterNode(node.id)}><span className="node-sigil"><MapNodeIcon type={node.type} /></span><span className="node-location">{locName}</span><strong>{presentation.label}</strong></button>; })}</div><div className="map-compass" aria-hidden="true"><b>N</b><svg viewBox="0 0 24 34"><path d="M12 2L20 28 12 23 4 28Z" /></svg></div><div className="map-scale" aria-hidden="true"><span /><b>{t('run.scaleUnit')}</b></div><span className="map-sheet-label">{t('run.mapBureau')}</span></div>
  <div className="map-legend"><span><i className="legend-line active" /> {t('run.legendOpen')}</span><span><i className="legend-line future" /> {t('run.legendFuture')}</span><span><i className="legend-line traveled" /> {t('run.legendTraveled')}</span><span><i className="legend-river" /> {t('run.legendRiver')}</span><span>{t('run.legendContour')}</span></div>{previewNode && <div className="node-preview panel" role="status"><p className="eyebrow">{(locale === 'en' ? localizedBiome(previewNode.regionId ?? map!.biomeId, locale).name : '') || biomeById(previewNode.regionId ?? map!.biomeId).name} · {localizedLocation(previewNode.regionId ?? map!.biomeId, previewNode.locationName, locale)} · {nodePresentation(previewNode, map!.availableNodeIds.includes(previewNode.id), map!.visitedNodeIds.includes(previewNode.id), map!.currentNodeId === previewNode.id, locale).label} · {t('run.elevation', { row: previewNode.row + 1 })}</p><div className="node-preview-scores"><span>{t('run.risk', { value: previewNode.risk })}</span><span>{t('run.reward', { value: previewNode.reward })}</span><span>{t('run.resource', { value: previewNode.resource })}</span></div></div>}</article>
  <aside className="panel stat-panel"><p className="eyebrow">{t('run.supplies')}</p><h2>{cl.characterName(character?.id ?? 'wanderer')}</h2><p className="character-signature">{cl.characterSig(character?.id ?? 'wanderer')}</p><div className="stat-row"><span>{t('run.statDifficulty')}</span><strong>{state.run?.ascensionLevel ?? 1}</strong></div><div className="stat-row"><span>{t('run.statHp')}</span><strong>{state.player?.hp}/{state.player?.maxHp}</strong></div><div className="stat-row"><span>{t('run.statGold')}</span><strong>{state.player?.gold}</strong></div><div className="stat-row"><span>{t('run.statDeck')}</span><strong>{t('run.statDeckUnit', { count: state.player?.deck.length ?? 0 })}</strong></div><div className="stat-row"><span>{t('run.statRelics')}</span><strong>{t('run.statRelicsUnit', { count: state.player?.relics.length ?? 0 })}</strong></div></aside></section><p className="phase-note">{t('run.seedNote')}</p></main>;
}
